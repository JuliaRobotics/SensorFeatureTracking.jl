...
const downsampleFactor_1 = 1        # downsampleFactor_2: use pyramid images (first track on a downsampled image, then on the regular image with a smaller windowsize), increases speed. Set to downsample_factor1 to use only regular image
const downsampleFactor_2 = 2
...
frame_counter_from = 51             # in an image sequence you can select a range of images from frame_counter_from to frame_counter_to
frame_counter_to = 99
...
corners = getApproxBestHarrisInWindow(I, nfeatures=9)   # function in RT_tracking_functions_list.jl
...
ITVar, ITConst = ImageTrackerSetup(orgI_setup, corners, windowSize = windowSize_20, downsampleFactor_setup = downsampleFactor_2)
